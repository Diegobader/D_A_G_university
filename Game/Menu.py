    ...
    def run(self, joystick):
        ...
        while True:
            ...
            for event in pygame.event.get():                   #QUe ocurre con las teclas
                if event.type == pygame.QUIT:
                    pygame.quit()
                if (event.type == pygame.KEYDOWN and event.key == pygame.K_UP) or joystick.get_axis(1) <= -0.5:
                    if choosi==1:
                        choosi=len(self.items)
                    else:
                        choosi-=1
                elif (event.type == pygame.KEYDOWN and event.key == pygame.K_DOWN) or joystick.get_axis(1) >= 0.5:
